...
rkJggg==
THINK;
 if (isset($_GET['phpinfo'])) { phpinfo(); exit(); } if (isset($_GET['img'])) { switch ($_GET['img']) { case 'pngFolder' : header("Content-type: image/png"); echo base64_decode($pngFolder); exit(); case 'pngFolderGo' : header("Content-type: image/png"); echo base64_decode($pngFolderGo); exit(); case 'gifLogo' : header("Content-type: image/gif"); echo base64_decode($gifLogo); exit(); case 'pngPlugin' : header("Content-type: image/png"); echo base64_decode($pngPlugin); exit(); case 'pngWrench' : header("Content-type: image/png"); echo base64_decode($pngWrench); exit(); case 'favicon' : header("Content-type: image/x-icon"); echo base64_decode($favicon); exit(); } } if (isset ($_GET['lang'])) { $langue = htmlspecialchars($_GET['lang'],ENT_QUOTES); if ($langue != 'en' && $langue != 'fr' ) { $langue = 'fr'; } } elseif (isset ($_SERVER['HTTP_ACCEPT_LANGUAGE']) AND preg_match("/^fr/", $_SERVER['HTTP_ACCEPT_LANGUAGE'])) { $langue = 'fr'; } else { $langue = 'en'; } $aliasContents = ''; if (is_dir($aliasDir)) { $handle=opendir($aliasDir); while (($file = readdir($handle))!==false) { if (is_file($aliasDir.$file) && strstr($file, '.conf')) { $msg = ''; $aliasContents .= '<li><a href="'.str_replace('.conf','',$file).'/">'.str_replace('.conf','',$file).'</a></li>'; } } closedir($handle); } if (empty($aliasContents)) $aliasContents = "<li>".$langues[$langue]['txtNoAlias']."</li>\n"; $handle=opendir("."); $projectContents = ''; while (($file = readdir($handle))!==false) { if (is_dir($file) && !in_array($file,$projectsListIgnore)) { $projectContents .= '<li><a href="'.($suppress_localhost ? 'http://' : '').$file.'">'.$file.'</a></li>'; } } closedir($handle); if (empty($projectContents)) $projectContents = "<li>".$langues[$langue]['txtNoProjet']."</li>\n";; $phpExtContents = ''; $loaded_extensions = get_loaded_extensions(); setlocale(LC_ALL,"<?php echo ($langues[$langue]['locale']); ?>"); sort($loaded_extensions,SORT_LOCALE_STRING); foreach ($loaded_extensions as $extension) $phpExtContents .= "<li>${extension}</li>"; $pageContents = <<<THINK
...
<body>
	...
	<ul class="utility">
		...
		<li><a href="?lang=<?php echo ($langues[$langue]['autreLangueLien']); ?>"><?php echo ($langues[$langue]['autreLangue']); ?></a></li>
	</ul>

	<h2> <?php echo ($langues[$langue]['titreConf']); ?> </h2>

	<dl class="content">
		<dt><?php echo ($langues[$langue]['versa']); ?></dt>
		<dd>${apacheVersion}&nbsp;&nbsp;-&nbsp;<a href='http://<?php echo ($langues[$langue][$doca_version]); ?>'>Documentation</a></dd>
		<dt><?php echo ($langues[$langue]['versp']); ?></dt>
		<dd>${phpVersion}&nbsp;&nbsp;-&nbsp;<a href='http://<?php echo ($langues[$langue]['docp']); ?>'>Documentation</a></dd>
		<dt><?php echo ($langues[$langue]['server']); ?></dt>
		<dd>${server_software}</dd>
		<dt><?php echo ($langues[$langue]['phpExt']); ?></dt> 
		<dd>
			<ul>
			${phpExtContents}
			</ul>
		</dd>
		<dt><?php echo ($langues[$langue]['versm']); ?></dt>
		<dd>${mysqlVersion} &nbsp;-&nbsp; <a href='http://<?php echo ($langues[$langue]['docm']); ?>'>Documentation</a></dd>
	</dl>
	<div style="margin-top:5px;border-top:1px solid #999;"></div>
	<div class="third left">
	<h2><?php echo ($langues[$langue]['titrePage']); ?></h2>
	<ul class="tools">
	...
	<h2><?php echo ($langues[$langue]['txtAlias']); ?></h2>
	<ul class="aliases">
	${aliasContents}			
	</ul>
	</div>
